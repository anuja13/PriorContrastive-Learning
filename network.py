...
class Network(nn.Module):
    ...
    def return_downstream_prob(self, x,  head='headless'):
        ...
        if head == 'linear':
            ...
            return self.classifier(feat)
        elif head == 'headless': # return features as such
            print('returning encoder features of size [64,2048] before linear layers')
    ...
    def forward(self, images=None, patches=None,  mode=0):
        ...
        if mode == 2:
            return self.return_downstream_prob(x=images, head='headless')  # Linear Eval without dropping conv layers
        # if mode == 3:
